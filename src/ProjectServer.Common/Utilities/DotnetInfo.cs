...
namespace MSBuildProjectTools.ProjectServer.Utilities
{
    ...
    public partial class DotnetInfo
    {
        ...
        /// <summary>
        ///     The regular expression used to parse runtime information from the output of "dotnet --list-runtimes".
        /// </summary>
        ...
        public static DotnetInfo GetCurrent(string? baseDirectory = null, bool enableDotnetHostDiagnostics = false, ILogger? logger = null)
        {
            ...
            try
            {
                ...
                using (dotnetOutput)
                {
                    ...
                    {
                        logger.LogError("Cannot resolve the target .NET SDK tooling or runtime for directory {BaseDirectory}. Please verify that the SDK version referenced in global.json (or a compatible runtime that matches the configured roll-forward policy) is correctly installed. Visit https://learn.microsoft.com/en-us/dotnet/core/tools/global-json#rollforward for more information.", baseDirectory);
                    ...
                    logger.LogTrace("Discovered .NET SDK v{SdkVersion:l}.", sdkVersion);
                }
                ...
                using (dotnetOutput)
                {
                    ...
                    if (targetSdk != null)
                        logger.LogTrace("Target .NET SDK is v{SdkVersion:l} in {SdkBaseDirectory}.", targetSdk.Version, targetSdk.BaseDirectory);
                    else
                        logger.LogError("Cannot find SDK v{SdkVersion} via 'dotnet --list-sdks'.", sdkVersion);
                }
                ...
                using (dotnetOutput)
                {
                    ...
                    if (hostRuntime != null)
                        logger.LogTrace(".NET host runtime is v{RuntimeVersion:l} ({RuntimeName}).", hostRuntime.Version, hostRuntime.Name);
                    else
                        logger.LogError("Failed to discover any runtimes via 'dotnet --list-runtimes'.");
                }

                return new DotnetInfo
                {
                    Runtime = hostRuntime ?? DotnetRuntimeInfo.Empty,
                    Sdk = targetSdk ?? DotnetSdkInfo.Empty,
                };
            }
            catch (Exception unexpectedError) when (unexpectedError is not DotnetInfoException)
            {
                logger.LogError(unexpectedError, "Unexpected error while attempting to discover target .NET runtime/SDK for directory {BaseDirectory}.", baseDirectory);
        ...
        static (DotNetHostExitCode ExitCode, TextReader StdOut) InvokeDotNetHost(string commandLineArguments, string baseDirectory, ILogger logger, bool enableHostTracing = false)
        {
            ...
            using (dotnetHostProcess)
            {
                ...
                logger.LogDebug("Launching {Command}...", command);

                dotnetHostProcess.Start();

                logger.LogDebug("Launched {Command}. Waiting for process {TargetProcessId} to terminate...", command, dotnetHostProcess.Id);
                ...
                if (!exited)
                {
                    logger.LogError("Timed out after waiting 5 seconds for {Command} to exit.", command);
                ...
                logger.LogDebug("Waiting for redirected STDOUT/STDERR streams to complete for process {TargetProcessId}...", dotnetHostProcess.Id);
                ...
                if (hostExitCode.IsWellKnownExitCode())
                    logger.LogDebug("{Command} terminated with exit code 0x{ExitCode:x} ({ExitCodeName}).", command, (int)hostExitCode, hostExitCode);
                else
                    logger.LogDebug("{Command} terminated with exit code 0x{ExitCode:x}.", command, (int)hostExitCode);
                ...
                if (!hostExitCode.IsSuccess() || logger.IsEnabled(LogLevel.Trace))
                {
                    if (!string.IsNullOrWhiteSpace(stdOut))
                        logger.LogDebug("{Command} returned the following text on STDOUT:\n\n{DotNetOutput:l}", command, stdOut);
                    else
                        logger.LogDebug("{Command} returned no output on STDOUT.", command);

                    if (!string.IsNullOrWhiteSpace(stdErr))
                        logger.LogDebug("{Command} returned the following text on STDERR:\n\n{DotNetOutput:l}", command, stdErr);
                    else
                        logger.LogDebug("{Command} returned no output on STDERR.", command);
                }
    ...
    /// </param>
    public record class DotnetSdkInfo(SemanticVersion Version, string BaseDirectory)
    {
        /// <summary>
        ///     Does the <see cref="DotnetSdkInfo"/> have a valid <see cref="Version"/>?
        /// </summary>
        public bool HasVersion => Version != EmptyVersion;

        /// <summary>
        ///     Does the <see cref="DotnetSdkInfo"/> have a valid <see cref="BaseDirectory"/>?
        /// </summary>
        public bool HasBaseDirectory => !String.IsNullOrWhiteSpace(BaseDirectory);

        /// <summary>
        ///     Does the <see cref="DotnetSdkInfo"/> represent an unknown or missing SDK?
        /// </summary>
        public bool IsEmpty => !HasBaseDirectory && !HasVersion;

        /// <summary>
        ///     Get the SDK version (if any) as a string.
        /// </summary>
        /// <returns>
        ///     The SDK <see cref="Version"/> as a string, or <c>null</c> if the SDK <see cref="Version"/> is empty.
        /// </returns>
        public string? GetVersionString() => HasVersion ? Version.ToString() : null;

        /// <summary>
        ///     The <see cref="SemanticVersion"/> representing an unknown SDK version.
        /// </summary>
        static readonly SemanticVersion EmptyVersion = new SemanticVersion(0, 0, 0);

        /// <summary>
        ///     <see cref="DotnetSdkInfo"/> representing an unknown or missing SDK.
        /// </summary>
        public static readonly DotnetSdkInfo Empty = new DotnetSdkInfo(Version: EmptyVersion, BaseDirectory: String.Empty);
    };
    ...
    ///     The SDK version.
    /// </param>
    public record class DotnetRuntimeInfo(string Name, SemanticVersion Version)
    {
        /// <summary>
        ///     Does the <see cref="DotnetRuntimeInfo"/> have a valid <see cref="Version"/>?
        /// </summary>
        public bool HasVersion => Version != EmptyVersion;

        /// <summary>
        ///     Does the <see cref="DotnetRuntimeInfo"/> have a valid <see cref="Name"/>?
        /// </summary>
        public bool HasName => !String.IsNullOrWhiteSpace(Name);

        /// <summary>
        ///     Does the <see cref="DotnetRuntimeInfo"/> represent an unknown or missing SDK?
        /// </summary>
        public bool IsEmpty => !HasName && !HasVersion;
        ...
        /// <summary>
        ///     The <see cref="SemanticVersion"/> representing an unknown runtime version.
        /// </summary>
        static readonly SemanticVersion EmptyVersion = new SemanticVersion(0, 0, 0);

        /// <summary>
        ///     <see cref="DotnetRuntimeInfo"/> representing an unknown or missing runtime.
        /// </summary>
        public static readonly DotnetRuntimeInfo Empty = new DotnetRuntimeInfo(Name: String.Empty, Version: EmptyVersion);
    };
